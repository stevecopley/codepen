...
(function (global, factory) {
   ...
   *
   * ----------------------------------------------------
   * MODIFIED BY STEVE COPLEY FOR VERTICAL RESIZING
   * ----------------------------------------------------
   */
  var PluginPen = function () {
    function PluginPen(jotted, options) {
      ...
      //---------------------------------------------------------------
      // Bodge for horiz resize result pane
      var $resultPane = jotted.$container.querySelector('.jotted-pane-result');

      this.resizableResult = {
      ...
      this.resizableResult.expander = document.createElement('div');
      this.resizableResult.expander.classList.add('jotted-plugin-pen-expander');
      this.resizableResult.expander.addEventListener('mousedown', this.startExpandResult.bind(this, jotted));

      $resultPane.insertBefore(this.resizableResult.expander, $resultPane.firstChild);
    }
    ...
    createClass(PluginPen, [{
      ...
      value: function startExpand(jotted, event) {
        ...
        var $relativePixel = 100 / parseInt(window.getComputedStyle($pane.container.parentNode)['height'], 10);

        // ugly but reliable & cross-browser way of getting height/width as percentage.
        $pane.container.parentNode.style.display = 'none';

        $pane.startY = event.clientY;
        $pane.startHeight = parseFloat(window.getComputedStyle($pane.container)['height'], 10);
        $previousPane.startHeight = parseFloat(window.getComputedStyle($previousPane.container)['height'], 10);

        $pane.container.parentNode.style.display = '';

        if( $pane.mousemove == undefined ) {
      ...
      value: function doDrag(pane, previousPane, relativePixel, event) {
        ...
        var ppNewHeight = previousPane.startHeight + (event.clientY - pane.startY) * relativePixel;

        // current pane new width
        var cpNewHeight = pane.startHeight - (event.clientY - pane.startY) * relativePixel;

        // contracting a pane is restricted to a min-size of 10% the container's space.
        var PANE_MIN_SIZE = 10; // percentage %
        if (ppNewHeight >= PANE_MIN_SIZE && cpNewHeight >= PANE_MIN_SIZE) {
          pane.container.style.maxHeight = 'none';
          pane.container.style.height = cpNewHeight + '%';
          previousPane.container.style.maxHeight = 'none';
          previousPane.container.style.height = ppNewHeight + '%';
        }
      }
    }, {
      key: 'stopDrag',
      value: function stopDrag(pane, event) {
        ...
        pane.mousemove = undefined;
        pane.mouseup = undefined;
      }
    }, {
      key: 'startExpandResult',
      value: function startExpandWidth(jotted, event) {
        var $pane = this.resizableResult;
        ...
        for (var i = 0; i < this.resizablePanes.length; i++) {
          this.resizablePanes[i].startWidth = parseFloat(window.getComputedStyle(this.resizablePanes[i].container)['width'], 10);
        }

        $pane.container.parentNode.style.display = '';

        if( $pane.mousemove == undefined ) {
          $pane.mousemove = this.doDragResult.bind(this, $relativePixel);
          document.addEventListener('mousemove', $pane.mousemove, false);
        }
        if( $pane.mouseup == undefined ) {
          $pane.mouseup = this.stopDrag.bind(this, $pane);
      ...
      key: 'doDragResult',
      value: function doDragResult(relativePixel, event) {
        var $pane = this.resizableResult;

        // other panes new width
        var opNewWidth = this.resizablePanes[0].startWidth + (event.clientX - $pane.startX) * relativePixel;
        ...
        // contracting a pane is restricted to a min-size of 10% the container's space.
        var PANE_MIN_SIZE = 10; // percentage %
        if (opNewWidth >= PANE_MIN_SIZE && cpNewWidth >= PANE_MIN_SIZE) {
          $pane.container.style.maxWidth = 'none';
          $pane.container.style.width = cpNewWidth + '%';
          for (var i = 0; i < this.resizablePanes.length; i++) {
            this.resizablePanes[i].container.style.maxWidth = 'none';
            this.resizablePanes[i].container.style.width = opNewWidth + '%';
          }
        }
      }
    }]);
